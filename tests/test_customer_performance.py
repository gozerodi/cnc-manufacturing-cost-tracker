...
def scenario():
    ...
    machine1_id = machine_service.create_machine(f"CPerf M1 {suffix}", D("100"))
    machine2_id = machine_service.create_machine(f"CPerf M2 {suffix}", D("200"))
    customer1_id = customer_service.create_customer(f"CPerf Customer1 {suffix}")
    customer2_id = customer_service.create_customer(f"CPerf Customer2 {suffix}")

    work_order1_id = planning_service.create_work_order(
        order_date=datetime.date(2026, 7, 24),
        customer_id=customer1_id,
        product_name=f"CPerf Product1 {suffix}",
        quantity=1,
    ...
    work_order2_id = planning_service.create_work_order(
        ...
        product_name=f"CPerf Product2 {suffix}",
        quantity=1,
        raw_material_cost=D("0"),
        profit_percent=D("0"),
        extra_cost=D("0"),
    ...
    yield {
        ...
        "customer1_name": f"CPerf Customer1 {suffix}",
        "customer2_name": f"CPerf Customer2 {suffix}",
        "work_order1_id": work_order1_id,
        "work_order2_id": work_order2_id,
        "product_name1": f"CPerf Product1 {suffix}",
    }
    ...
    try:
        ...
        session.query(Customer).filter(
            Customer.id.in_([customer1_id, customer2_id])
        ).delete(synchronize_session=False)
        session.commit()
    finally:
        session.close()
...
def test_customer_performance_matches_manual_calculation(scenario):
    ...
    results = analytics_service.list_customer_performance(
        datetime.date(2026, 7, 24), datetime.date(2026, 7, 24)
    )
    c1 = next(r for r in results if r.customer_id == scenario["customer1_id"])
    c2 = next(r for r in results if r.customer_id == scenario["customer2_id"])

    assert c1.total_price_contribution == D("300.0000")
    assert c1.hourly_return == D("120.0000")
    assert c2.total_price_contribution == D("200.0000")
    assert c2.hourly_return == D("200.0000")
    assert results.index(c2) < results.index(c1)


def test_customer_performance_respects_date_range(scenario):
    step1 = production_service.list_steps_for_work_order(scenario["work_order1_id"])[0]
    ...
    c1 = next(r for r in results if r.customer_id == scenario["customer1_id"])
    assert c1.total_price_contribution == D("100.0000")


def test_customer_detail_filters_by_customer_and_date(scenario):
    step1 = production_service.list_steps_for_work_order(scenario["work_order1_id"])[0]
    step2 = production_service.list_steps_for_work_order(scenario["work_order2_id"])[0]
    ...
    entries = analytics_service.list_customer_detail(
        scenario["customer1_id"], datetime.date(2026, 7, 24), datetime.date(2026, 7, 24)
    )
